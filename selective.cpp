...
void selective::send_lost_function(int num) {
	float time;
	cout << "\nflag : " << flag;
	char r;
	char s;
	...
	if (check) {
		r = '1';
		recive_list << 1 << endl;
		Time_Array[num] = (chrono::duration<float>(chrono::system_clock::now() - current_time).count()) * 1000;
		all_number_sent_bits++;
	}
	else if (!check) {
		r = '0';
...
void selective::print(char send_item, char recive_item)
{
