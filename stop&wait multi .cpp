...
Stop_Wait_multi::Stop_Wait_multi(int bits)
{
	flag = 0;
	lost_bits = 0;
	bits_num = bits;
...
void Stop_Wait_multi::send_function()
{
		...
		while (pass) {
			...
			if (check_send) {
				...
				if (check1 == true && check2 == true) {
					...
					lost_bits++;
					if (check1)
						cout << "recive 1 succesfully" << endl;
					if (!check1)
						cout << "recive 1 fuckedup ! " << endl;
					if (check2)
						cout << "recive 2 succesfully" << endl;
					if (!check2)
						cout << "recive 2 fuckedup ! " << endl;
...
bool Stop_Wait_multi::recive_check()
{
	if (rand() % 3 == 0)
		return false;
	return true;
...
void Stop_Wait_multi::remove_line(int input)
{
	fstream main_file;
...
void Stop_Wait_multi::read_from_file()
{
	string temp;
...
void Stop_Wait_multi::get_result()
{
...
void Stop_Wait_multi::get_array()
{
	for (int i = 0; i < bits_num; i++)
		cout << main_array[i] << endl;
}

void Stop_Wait_multi::sleeping(int seconds)
{
	clock_t endwait;
...
bool Stop_Wait_multi::send_check()
{
	if (rand() % 5 == 0)
		return false;
	return true;
